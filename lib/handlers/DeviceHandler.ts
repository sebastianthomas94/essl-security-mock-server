...
export class DeviceHandler {
  ...
  handlePolling(deviceSN: string): string {
    ...
    const commandStrings = pendingCommands.map(
      (cmd: any) => `C:${cmd.id}:${cmd.command}`
    );
    this.commandManager.markCommandsAsSent(deviceSN, pendingCommands);

    // Emit command sent events
    pendingCommands.forEach((cmd: any) => {
  ...
  handleDataRequest(table: string, deviceSN: string, queryParams: any): string {
    ...
    if (table === "USERINFO") {
      ...
      console.log(
        `📄 No local user data for device ${deviceSN} - users stored on device only`
      );
      return "OK";
    }
  ...
  handleDataSubmission(
    ...
    if (table === "ATTLOG") {
      const lines = rawBody
        ...
        .map((l) => l.trim())
        .filter(Boolean);
      const attendanceRecords = lines.map((line) =>
        this.parser.parseAttendanceLine(line)
      );
    ...
    if (table === "OPERLOG") {
      ...
      const fingerPrint = fingerprintLines.map((line) =>
        this.parser.parseFingerprintLine(line)
      );
      const operation = operationLines.map((line) =>
        this.parser.parseOperationLine(line)
      );
      ...
      if (users.length > 0) {
        ...
        console.log(
          `📄 Received ${users.length} users from device ${deviceSN} `
        );
      }
  ...
  handleCommandAck(body: any): void {
    ...
    const acknowledged = this.commandManager.acknowledgeCommand(
      SN,
      String(ID),
      Return === "0" ? "OK" : "ERR"
    );
