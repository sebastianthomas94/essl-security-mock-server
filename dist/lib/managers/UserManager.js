...
export class UserManager {
    constructor(commandManager, dataParser, eventManager, defaultDeviceSN, logRequests) {
        this.commandManager = commandManager;
        this.dataParser = dataParser;
        this.eventManager = eventManager;
        this.defaultDeviceSN = defaultDeviceSN;
    ...
    addUser(userData) {
        ...
        const command = this.commandManager.queueCommand(this.defaultDeviceSN, `DATA UPDATE USERINFO ${this.dataParser.formatUserForDevice(user)}`);
        this.commandManager.queueCommand(this.defaultDeviceSN, "DATA QUERY USERINFO");
        if (this.logRequests) {
            console.log(`👤 User ${user.Name} (PIN: ${user.PIN}) added to device ${user.deviceSN} - stored on device only`);
        }
    ...
    addUsers(usersData, defaultDeviceSN = "DEFAULT") {
        try {
            ...
            this.commandManager.queueCommand(this.defaultDeviceSN, "DATA QUERY USERINFO");
            if (this.logRequests) {
                console.log(`👥 ${addedUsers.length} users added to devices - stored on devices only`);
            }
            return {
                success: true,
                data: {
     ...
     */
    getUserByPin(pin, deviceSN = "DEFAULT") {
        this.commandManager.queueCommand(this.defaultDeviceSN, `DATA QUERY USERINFO PIN=${pin}`);
        console.log(`Query user PIN ${pin} from device ${deviceSN} - no local storage`);
        return null;
    }
    getUsers(deviceSN = "DEFAULT") {
        this.commandManager.queueCommand(this.defaultDeviceSN, `DATA QUERY USERINFO *`);
     ...
     */
    updateUser(pin, updateData, deviceSN = "DEFAULT") {
        ...
        const command = this.commandManager.queueCommand(this.defaultDeviceSN, `C:${pin}:DATA UPDATE USERINFO  PIN=${pin}\tName=${updateData.Name}`);
        if (this.logRequests) {
            console.log(`👤 User PIN ${pin} updated on device ${deviceSN} - no local storage`);
        }
        return { success: true, data: { user, command } };
    }
    /**
     ...
     */
    deleteUser(pin) {
        const command = this.commandManager.queueCommand(this.defaultDeviceSN, `DATA DELETE USERINFO PIN=${pin}`);
        this.commandManager.queueCommand(this.defaultDeviceSN, "DATA QUERY USERINFO");
    ...
    deleteAllUsers() {
        ...
        const command = this.commandManager.queueCommand(this.defaultDeviceSN, "DATA DELETE USERINFO");
        this.commandManager.queueCommand(this.defaultDeviceSN, "DATA QUERY USERINFO");
        if (this.logRequests) {
            console.log(`👥 All users deleted from device ${this.defaultDeviceSN} - no local storage`);
        }
        return {
            success: true,
     ...
     */
    deleteAllUsersFromAllDevices(deviceSN) {
        // This would need a list of known devices in a real implementation
        // For now, just delete from DEFAULT device
        const command = this.commandManager.queueCommand(deviceSN, "DATA DELETE USERINFO");
        this.eventManager.emit("users.allDeletedFromAllDevices", { command });
        if (this.logRequests) {
            console.log(`👥 Delete all users command sent to all devices - no local storage`);
        }
        return {
            success: true,
            data: { command },
